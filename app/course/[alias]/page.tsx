...
function CourseHome({ params }: { params: { alias: string } }) {
  ...
  const [courseAlertConfig, setCourseAlertConfig] = useState({
    message: '',
    severity: ''
  })
  const [modulesAlertOpen, setModulesAlertOpen] = useState(false)
  const [modulesAlertConfig, setModulesAlertConfig] = useState({
    message: '',
    severity: ''
  })
  ...
  const {
    ...
    error: courseError
  } = useCourse(params.alias, userTokens.access)
  const { isLoading: modulesIsLoading, error: modulesError } = useModulesList(
    params.alias,
    userTokens.access
  )
  ...
  const handleModulesAlertOpen = (status: number) => {
    if (status === API_STATUS_CODE.NOT_FOUND) {
      ...
      setModulesAlertConfig({
        message:
          'Hubo un error al intentar acceder a los módulos. Intentalo de nuevo más tarde.',
        severity: 'error'
      })
      setModulesAlertOpen(true)
    }
      ...
      <Box component='section' className={styles.courseHomeContainer}>
        ...
        <Box component='section' className={styles.courseHomeSection}>
          <Box component='div' className={styles.courseDetailsCard}>
            ...
            <Box component='div' className={styles.courseDetailsSection}>
              <Typography component='p'>
                Comentarios: {courseData.comments}
              </Typography>
              <Typography component='p'>
                Interacciones: {courseData.appraisals}
              </Typography>
            </Box>
            <Box component='div' className={styles.courseDetailsSection}>
              <Typography component='p'>
                Estrellas: {courseData.avarage_stars ?? 0}
              </Typography>
              <Typography component='p'>
                Inscritos: {courseData.enrollments.length}
              </Typography>
            </Box>
            <Box component='div' className={styles.courseDetailsSection}>
              <Typography component='p'>
                Institución:
                {courseData.institution ?? 'Institución no encontrada'}
              </Typography>
            ...
            <Typography component='p'>
              ...
              nesciunt dicta saepe at vel! Nesciunt sint facere quos ducimus
              laudantium, ratione exercitationem praesentium ad odio, suscipit
              non consectetur ut!
            </Typography>
          </Box>
        </Box>
